        ...
        <div class="bg-img" style="background-color: #fff;background-image: url({{ asset('storage/screen-backgrounds/' . $meeting_hall_screen->background_name . '.' . $meeting_hall_screen->background_extension)}} ); height:100%; width:100%; background-size: cover; background-repeat: no-repeat;">
            ...
            @if($chair)
                <div class="d-flex align-items-center">
                    <h1 class="text-center w-100" id="chair" style="font-size: {{ $meeting_hall_screen->font_size }}vw; color: {{ $meeting_hall_screen->font_color }}; font-family: '{{ $meeting_hall_screen->font }}'; padding-top: 12%; font-weight: 900">{{ isset($chair->title) ? $chair->title . ' ' : null }}{{ $chair->first_name }} {{ $chair->last_name }}</h1>
                </div>
            @else
                <div class="d-flex align-items-center">
