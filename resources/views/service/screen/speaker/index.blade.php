    ...
    @endisset
        @if($speaker)
            <div class="d-flex align-items-center px-2">
                <h1 class="text-center w-100" id="speaker" style="font-size: {{ $meeting_hall_screen->font_size }}vw; color: {{ $meeting_hall_screen->font_color }}; font-family: '{{ $meeting_hall_screen->font }}'; padding-top: 36%; font-weight: 900">{{ isset($speaker->title) ? $speaker->title . ' ' : null }}{{ $speaker->first_name }} {{ $speaker->last_name }}</h1>
            </div>
        @else
            <div class="d-flex align-items-center px-2">
                <h1 class="text-center w-100" id="speaker" style="font-size: {{ $meeting_hall_screen->font_size }}vw; color: {{ $meeting_hall_screen->font_color }}; font-family: '{{ $meeting_hall_screen->font }}'; padding-top: 36%; font-weight: 900">...</h1>
                </h1>
            </div>
        @endif
    @isset($meeting_hall_screen->background_name)
