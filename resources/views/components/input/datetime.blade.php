...
<div class="col form-group mb-3">
    ...
    <input type="text" name="{{ $name }}" class="date-time-picker form-control @error($name)is-invalid @enderror" id="{{ $method }}-{{ $name }}" value="{{ old($name) }}" placeholder="{{ __('common.'.$title) }}" autocomplete="false" />
    @error($name)
