...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-white"><a href="{{ route('portal.meeting.participant.index', ['meeting' => $meeting->id]) }}" class="text-decoration-none text-white">{{ __('common.participants') }}</a></li>
    <li class="breadcrumb-item active text-white text-decoration-underline" aria-current="page">{{ $participant->full_name . ' | ' . __('common.participant-logs') }}</li>
@endsection
@section('body')
    <div class="card text-bg-dark">
        <div class="card-header">
            <h1 class="m-0 text-center"><span class="fa-duotone fa-screen-users fa-fade"></span> <small>"{{ $meeting->title }}"</small> {{ $participant->full_name . ' | ' . __('common.participant-logs') }}</h1>
        </div>
