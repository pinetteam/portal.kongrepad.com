...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('portal.meeting.show', $title->program->hall->meeting->id) }}" class="text-decoration-none">{{ $title->program->hall->meeting->title }}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('portal.meeting.report.debate.index', ['meeting' => $title->program->hall->meeting->id]) }}" class="text-decoration-none">{{ __('common.debate-reports') }}</a></li>
    <li class="breadcrumb-item active" aria-current="page">{{ '"' . $title->title. '" ' . __('common.participants') }}</li>
@endsection
