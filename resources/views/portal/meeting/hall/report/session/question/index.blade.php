...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item text-white"><a href="{{ route('portal.meeting.hall.show', ['meeting' => $session->program->hall->meeting->id, 'hall' => $session->program->hall->id]) }}" class="text-decoration-none text-white">{{ $session->program->hall->title }}</a></li>
    ...
    <li class="breadcrumb-item active text-white text-decoration-underline" aria-current="page">{{ $session->title . ' ' . __('common.question-reports') }}</li>
@endsection
@section('body')
